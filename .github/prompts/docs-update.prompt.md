...
|------|------------------|----------------|----------|
| **プロジェクト概要** | `README.md`, `index.md` | 概要、インストール、使用方法 | 完全性重視 + 自動更新対応 |
...
#### 特別処理: README.md自動更新機能
README.mdの更新時は以下の自動更新オプションを提供：

1. **手動更新**: 従来通りの手動編集
2. **半自動更新**: スクリプト生成 + 手動確認
3. **完全自動更新**: GitHub Actions経由の自動生成

**自動更新トリガー条件**:
- プロンプトファイル変更（`.github/prompts/**`）
- コマンド設定変更（`.copilot/commands.json`）
- Issueテンプレート変更（`.github/ISSUE_TEMPLATE/**`）
...
#### 3.2 README.md特別処理
README.mdの更新時は、ユーザーの意図に応じて以下から選択：
...
# 現在の構造確認
head -30 README.md
grep "^#" README.md # セクション一覧確認
```

**選択基準**:
- 構造的変更（コマンド追加/削除）→ 自動更新推奨
- 内容的変更（説明文の修正）→ 手動更新推奨
- 初回作成 → 自動更新で骨組み作成後、手動調整
